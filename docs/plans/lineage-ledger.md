...
- `lineage.spawn` is by entry and inherits the prompt only — and since packet 17 of `docs/plans/child-source.md` (2026-09-21) the child's *executor* is shown the parent's kept `sketch.js` under a heading in its brief, recorded per attempt in `attempts.given_source_json` with the context window in `attempts.num_ctx`. `spawn` itself still copies no code: the prompt is the whole of what the row carries, the source is read off disk by `worker.source_for` at the top of each attempt, and `meta.executor_source` (or the job's own `executor_source`, migration 017) can turn it off. So "spawn from attempt 2" still changes only which sketch was looked at — but it now has a second reader: §8.3's `parent_attempt` would choose which attempt's sketch the child's executor is *shown*, not just which one the critic saw, and the entry page and `meta.json` say which it was (`lineage.inherits_source`, the **Revised from** row). Still worth recording, still not worth a different job. *Rewritten 2026-09-22, packet 18.*
