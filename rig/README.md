...
| ghost window | last of all, a pointer script — at most 64 events over at most 8,000 ms, yours from a `ghost` block or the built-in your assertions name — played through real mouse input and written as `ghost.png`. It decides nothing: every check and every assertion is settled before it opens | `gate/sketch_gate.py` `GHOST_MAX_EVENTS`, `GHOST_MAX_MS`, `GHOST_BUILTINS` |
| images | every image that arrives from a host outside the sketch is recorded — at most 8 of them — as host, type, bytes and ms; `loads(image)` passes when that list is not empty and the canvas is not one flat colour. A `data:` URI is not the web and does not pass | `gate/sketch_gate.py` `MAX_RESOURCES_LOADED`, `evaluate_assertion` |
