...
Three sampling decisions the fixtures and the first repair run forced, stated
up front:
...
Exit codes follow the delegate.py convention used elsewhere in this repo:
    ...
    1  console_clean, frame_advancing, sound_lib_ok or audio_context_running
       reported false, or a requested assertion failed.  is_looping is a
       declaration, not a verdict, and never fails a run on its own.
...
def step(page, n, chunk=30, settle_ms=25):
