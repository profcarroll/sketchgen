...
run "restart after a freeze"       systemsetup -setrestartfreeze on
...
run "time zone"                    systemsetup -settimezone America/New_York
run "network time"                 systemsetup -setusingnetworktime on
...
D=/etc/ssh/sshd_config.d; F=$D/010-sketchgen-kiosk.conf
...
EOF
    if out=$(/usr/sbin/sshd -t 2>&1); then
        row "ssh: keys, tailnet, $U only" "ok ($F)"
    else
        rm -f $F; row "ssh: keys, tailnet, $U only" "FAILED: sshd -t: $(echo "$out" | head -1); removed"
    fi
else
    row "ssh: keys, tailnet, $U only" "FAILED: /etc/ssh/sshd_config has no Include line; not turning Remote Login on"
    F=
fi
if [ -n "$F" ] && [ -f "$F" ]; then
